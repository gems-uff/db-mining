...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Gera as saidas da RQ1 a partir de base_rqs.csv."
    )
    parser.add_argument(
        "--input-dir",
        default=DEFAULT_INPUT_DIR,
        help="Diretorio com base_rqs.csv.",
    )
...
def main() -> None:
    ...
    if "is_vulnerable" not in base_df.columns:
        raise ValueError("base_rqs.csv deve conter a coluna is_vulnerable.")
    ...
    assoc_path = input_dir / "rq1_associacoes.csv"
    projects_path = input_dir / "rq1_projetos.csv"
    dbms_path = input_dir / "rq1_dbms.csv"
